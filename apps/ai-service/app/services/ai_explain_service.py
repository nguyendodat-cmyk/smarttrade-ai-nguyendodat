...
VIETNAMESE_TEMPLATES: Dict[str, str] = {
    ...
    # Volume Analysis
    "VA01": (
        "Breakout với khối lượng cao gấp {volume_ratio:.1f} lần trung bình, "
    ...
    "VA02": (
        ...
        "Phân kỳ giá-khối lượng, tín hiệu yếu."
    ),
    "VA03": (
        "Khối lượng đạt đỉnh ({volume:,}), nằm trong top 5% của 20 phiên gần nhất. "
        "Có thể là tín hiệu đảo chiều hoặc bùng nổ."
    ),
    # Technical / Momentum
    "TM02": (
        "{cross_type_vi}: MA20 ({ma20:,.0f}) cắt {cross_direction} MA50 ({ma50:,.0f}). "
        "Tín hiệu {signal_vi} trung hạn."
    ),
    "TM04": (
        "RSI đạt {rsi14:.1f} (quá mua, >70). "
        "Cổ phiếu có thể đã tăng quá nhanh, cân nhắc chốt lời."
    ),
    "TM05": (
        "RSI xuống {rsi14:.1f} (quá bán, <30). "
        "Cổ phiếu có thể đã giảm quá sâu, cân nhắc mua vào."
    ),
}
...
class AIExplainService:
    ...
    async def _explain_llm(self, event: InsightEvent) -> Optional[str]:
        ...
        prompt = (
            f"Bạn là chuyên gia phân tích kỹ thuật chứng khoán Việt Nam.\n"
            f"Hãy giải thích ngắn gọn (2-3 câu, tiếng Việt) tín hiệu sau:\n\n"
            f"Mã: {event.symbol}\n"
